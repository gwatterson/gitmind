...
async def reject_review(review_id: str):
    """Reject findings and trigger posting REQUEST_CHANGES to GitHub (HITL)."""
    review = await crud.get_review(review_id)
    if not review:
        raise HTTPException(status_code=404, detail="Review not found")
    if review["status"] != "hitl_pending":
    ...
    # Force verdict to request_changes locally
    await crud.update_review(review_id, verdict="request_changes")
    ...
    gh = _get_github_client()
    if not gh:
        github_warning = "GitHub client not configured — review rejected locally but not posted to GitHub."
        log.warning("reject_without_github", review_id=review_id)
    else:
        try:
            repo = gh.get_repo(review["repo"])
            pr = repo.get_pull(review["pr_number"])
            ...
            # Post summary review rejecting the PR
            pr.create_review(
                body=review.get("summary", "Review completed by GitMind. Critical issues found, please fix them."),
                event="REQUEST_CHANGES"
            )
            github_posted = True
        except Exception as e:
            log.error("github_post_error", review_id=review_id, error=str(e))
            github_warning = f"Review rejected locally but GitHub posting failed: {str(e)}"

    await crud.update_review(review_id, status="completed", completed_at="datetime('now')")
    await crud.create_event(
        review_id=review_id,
        event_type="hitl_rejected",
        message="Review rejected by user" + (" and posted to GitHub." if github_posted else " (local only)."),
    )

    result = {"status": "rejected", "review_id": review_id, "github_posted": github_posted}
    if github_warning:
        result["warning"] = github_warning
    return result
